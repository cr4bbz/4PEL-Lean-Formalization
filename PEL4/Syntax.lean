...
/-- Syntactic macro for OR (Disjunction) via De Morgan. -/
def Formula.or {Atom Ag : Type} (phi psi : Formula Atom Ag) : Formula Atom Ag :=
  Formula.not (Formula.and (Formula.not phi) (Formula.not psi))

/-- Syntactic macro for IMPLIES (Material Implication). -/
def Formula.implies {Atom Ag : Type} (phi psi : Formula Atom Ag) : Formula Atom Ag :=
  Formula.or (Formula.not phi) psi
...
/-- ST-Entails (which strictly represents safe LP-Entailment). -/
