...
theorem gate70_stress_preferred_iff (costHundredths : Nat) :
    gate70CostPhase costHundredths = .sampleStress ↔ costHundredths < gate70CriticalCostHundredths := by
  simp [gate70CostPhase]

theorem gate70_sample_phase_iff (costHundredths : Nat) :
    gate70CostPhase costHundredths = .sampleStress ↔ costHundredths < gate70CriticalCostHundredths :=
  gate70_stress_preferred_iff costHundredths

theorem gate70_stop_phase_iff (costHundredths : Nat) :
    gate70CostPhase costHundredths = .stop ↔ ¬ costHundredths < gate70CriticalCostHundredths := by
  simp [gate70CostPhase]
...
theorem gate70_boundary_is_indifference :
    gate70StressNetValue ((2 : Rat) / 5) = gate70AliasStopValue := by native_decide
...
theorem gate70_recovers_gate69_stress_q :
    gate70StressNetValue ((1 : Rat) / 5) = sequentialExperimentQ gate69Model 0 gate66AliasedPrior .stressProbe := by
  native_decide
