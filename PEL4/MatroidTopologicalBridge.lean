...
theorem topologicalClosureExchange_iff_r0Like
  ...
  · intro hExchange x y hxy
    have hInsert :
      ...
      have hUnion :
          s.closure
              (regionUnion (EvidenceSet.singleton y) emptyEvidenceSet) x :=
        (closure_union_at s (EvidenceSet.singleton y) emptyEvidenceSet x).2
          (Or.inl hxy)
      simpa [EvidenceSet.insert, EvidenceSet.singleton, regionUnion,
        emptyEvidenceSet] using hUnion
    have hResult := hExchange hInsert (closure_empty_not_mem s x)
    have hUnionResult :
        s.closure
            (regionUnion (EvidenceSet.singleton x) emptyEvidenceSet) y := by
      simpa [EvidenceSet.insert, EvidenceSet.singleton, regionUnion,
        emptyEvidenceSet] using hResult
    have hSplit :=
  ...
  · intro hR0 A x y hInsert hx
    have hUnion :
        s.closure (regionUnion (EvidenceSet.singleton y) A) x := by
      simpa [EvidenceSet.insert, EvidenceSet.singleton, regionUnion] using hInsert
    have hSplit :=
      (closure_union_at s (EvidenceSet.singleton y) A x).1 hUnion
    cases hSplit with
    | inl hSingleton =>
        ...
        have hUnionReverse :
          ...
          (closure_union_at s (EvidenceSet.singleton x) A y).2
            (Or.inl hReverse)
        simpa [EvidenceSet.insert, EvidenceSet.singleton, regionUnion] using
          hUnionReverse
    | inr hA =>
