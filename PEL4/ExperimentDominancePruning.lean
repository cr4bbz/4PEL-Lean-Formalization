...
def Gate72QDominates (remaining : Nat) (belief : BayesianEpistemicBelief Gate47State)
...
def gate72PrunedStressValue (remaining : Nat)
...
theorem gate72_stress_dominates_cheap_one_step :
  ...
  unfold Gate72QDominates; native_decide
...
theorem gate72_pruning_preserves_one_sample_value :
    gate72PrunedStressValue 0 gate66AliasedPrior = sequentialExperimentValue gate69Model 1 gate66AliasedPrior := by
  native_decide

theorem gate72_pruning_preserves_two_sample_value :
    gate72PrunedStressValue 1 gate66AliasedPrior = sequentialExperimentValue gate69Model 2 gate66AliasedPrior := by
  native_decide
...
theorem gate72_experiment_dominance_and_pruning :
    ...
    gate72PrunedStressValue 0 gate66AliasedPrior = sequentialExperimentValue gate69Model 1 gate66AliasedPrior ∧
    gate72PrunedStressValue 1 gate66AliasedPrior = sequentialExperimentValue gate69Model 2 gate66AliasedPrior := by
  exact ⟨gate72_stress_dominates_cheap_one_step, gate72_stress_dominates_precise_one_step,
    gate72_stress_dominates_status_two_step.1, gate72_stress_dominates_status_two_step.2,
    gate72_pruning_preserves_one_sample_value, gate72_pruning_preserves_two_sample_value⟩
end PEL4
