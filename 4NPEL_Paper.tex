...
\begin{enumerate}
    ...
    \item I demonstrate that mapping non-bivalent validity standards (ST, LP, SS) onto this probabilistic space successfully models and defuses five classical epistemic paradoxes (Liar, Lottery, Preface, Moore's Paradox, and G\"odel's Incompleteness) alongside the threat of Epistemic Explosion.
...
\begin{tabular}{lccc}
\hline
\textbf{Framework} & \textbf{Handles Probability} & \textbf{Handles Contradiction} & \textbf{Derives Boundary Theorem} \\
\hline
...
By embedding non-bivalent validity standards into this probabilistic framework, I can actively resolve epistemic paradoxes rather than merely halting at contradictions. The Lean 4 formalization explicitly classifies these paradoxes into a structural taxonomy based on their Kripke attractors:
\begin{itemize}
    \item \textbf{Class I (Self-Reference):} Paradoxes like the Liar and the Epistemic G\"odel Sentence forcefully absorb the system into a stable \textbf{Epistemic Glut} ($B$).
    \item \textbf{Class II (Aggregation):} Paradoxes like the Lottery and Preface organically degrade beliefs, collapsing them into \textbf{Strict Falsity} ($F$) or Gaps.
    \item \textbf{Class III (Introspection):} Paradoxes like Moore's sentences fail self-reflection, collapsing deterministically into an \textbf{Epistemic Gap} ($N$).
\end{itemize}

Table \ref{tab:paradox_resolution} summarizes the mapping of classical failures to 4-PEL resolutions. The following subsections formalize these case studies, supported by Lean 4 formalizations of the core semantic and probabilistic constraints.
...
G\"odel's Incompleteness & Incompleteness (Gap) & Epistemic Glut ($B$) \\
...
\hline
\end{tabular}
\caption{Resolution of epistemic paradoxes in 4-PEL compared to classical Probabilistic Epistemic Logic.}
...
\end{equation*}
Since the intersection of $\{w'_{T, F, N}\}$ with $\mathcal{E}^+(E)$ is empty, their numerator is $0$. The agent's probability mass is entirely stripped from $T, F,$ and $N$. Since $P_B > 0$, the denominator consists solely of $P_B$, ensuring it is well-defined (non-zero). The probability mass must re-normalize over the remaining states in $\mathcal{E}^+(E)$. Because only $B$ remains, the fraction evaluates to $1$. Thus, $P_B^*(p, w) = 1$ becomes the unique stable state instantaneously.
...
While this is not a formal treatment of arithmetic incompleteness, it provides a powerful case study: a probabilistically resilient agent does not necessarily ``halt'' at self-referential unprovability; instead, it can synthesize the unprovable self-referential truth into a state of \textit{Overcompleteness} (an Epistemic Glut), actively holding a contradictory but mathematically stable belief.
\end{proof}
...
While a classical PEL can model this, 4-PEL is essential here because it provides a unified algebraic space where the aggregated conclusion organically degrades into a non-bivalent epistemic state without breaking the formal machinery. The system explicitly flags the conjunction as probabilistically degraded, aligning perfectly with standard probabilistic epistemologies while formalizing the breakdown of aggregation within the 4-valued algebra.
...
For any isolated epistemic agent $i$ with a constant threshold $c_i$, if a proposition breaches the first-order paradox boundary ($P_B \ge 2c_i - 1$), it instantaneously and necessarily floods the entire infinitely deep meta-belief hierarchy. Formally:
...
This provides a route to completeness by reduction to classical PEL, assuming a fully compositional translation for all modal and probabilistic formulas, including nested $B_i$ operators and agent-indexed thresholds.
\end{proposition}
...
Crucially, the threshold belief operator $B_i(\phi)$ translates directly to a classical probability inequality via this recursion: $\tau^+(B_i(\phi)) := P(\tau^+(\phi)) \ge c$. Because the translated formulas are strictly classical PEL formulas constructed over linear weight inequalities, 4-PEL inherits the standard Fagin-Halpern canonical model construction. Thus, any 4-PEL formula valid in all models is provable by translating it to classical PEL, applying the complete classical PEL axioms, and translating back. This guarantees both decidability and a reduction pathway to strong completeness. The direct, axiomatic completeness of the dynamic conditionalization fragment remains an open area for future work.
\end{proof}
...
Furthermore, by applying these non-bivalent validities to probabilistic belief operators, 4-PEL provides a robust mechanism to evaluate and defuse four major epistemic paradoxes, alongside the structural threat of epistemic explosion. The framework also demonstrates multi-agent rational disagreement. The seamless transition of classical contradictions into rational epistemic gaps or bounded gluts showcases the framework's expressive power. 

\section{Future Work}
\label{sec:futurework}

Future research will explore extending 4-PEL into full Dynamic Epistemic Logic (DEL) \cite{baltag2008probabilistic} with relational updates. Additionally, the polynomial-time model checking complexity suggests potential applications in artificial intelligence. As AI agents aggregate crowdsourced datasets, they encounter inherently contradictory training data. Implementing 4-PEL as a foundational layer could allow artificial systems to safely compartmentalize contradictions and maintain paraconsistent reasoning under uncertainty.
