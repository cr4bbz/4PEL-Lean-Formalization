...
The two implications make the remaining obstruction regime-dependent rather than
merely one-sided.

\begin{corollary}[Exact regime reduction]
\label{cor:threshold-regime-reduction}
For \(p+n=1\),
\[
  c\le\frac12
  \quad\Longrightarrow\quad
  \mathsf{ThresholdRegular}
  \Longleftrightarrow
  \mathsf{ThresholdConsistent},
\]
whereas
\[
  c>\frac12
  ...
  \mathsf{ThresholdRegular}
  \Longleftrightarrow
  \mathsf{ThresholdComplete}.
\]
\statusverified
\end{corollary}

Thus the critical value separates which half of regularity still needs to be checked.
At or below one half, underdecision is ruled out and only overacceptance can spoil
regularity.  Above one half, overacceptance is ruled out and only underdecision can
spoil regularity.
