...
No strong novelty claim is made in this version. The individual mathematical ingredients are elementary: Boolean pairs, threshold comparisons, affine interpolation, linear order, and midpoint arguments. The research contribution currently established internally to the project is the machine-checked composition of these ingredients into one dependency chain connecting admissible probabilistic update to four-valued categorical reachability, robustness, crossing order, and intermediate phase structure. A systematic literature audit is publication-critical.

\subsection{Open problem 1: model-path realizability}

The sharpest current boundary lies between the support trajectory and the model semantics.

\begin{question}[Admissible model-path realization]
Given an admissible conditionalization with pre-update and post-update support pairs, can the affine support interpolation
\[
  (\Ppos_t,\Pneg_t)
\]
be realized by a family of full 4-PEL models \(M_t\) satisfying the model constraints for all \(t\in[0,1]\cap\mathbb{Q}\), while preserving the intended structural fields?
\end{question}

A positive answer would upgrade the verified intermediate support phase to an actual intermediate model state. It would justify literal model-dynamic readings such as
\[
  \Nval\to\Tval\to\Bval
\]
rather than only the present thresholded support-path reading.

A subtlety is that several distinct realization notions are possible. One may interpolate local probability functions directly, require every intermediate model to arise from an admissible conditionalization of the original model, or construct a parametrized sequence of evidence events. These are mathematically different claims and should not be conflated.
