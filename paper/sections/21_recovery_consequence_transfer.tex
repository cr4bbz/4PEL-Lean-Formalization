...
each point to evaluations where the antecedent is classical. No classicality premise
on the consequent is needed. On this \emph{antecedent-classical-restricted} domain,
the ST and LP definitions again coincide. This is a semantic restriction of their
domains; it is not an assertion that unrestricted ST and LP consequence coincide over
all four-valued models. Unrestricted LP consequence always implies its
antecedent-classical restriction because the latter tests fewer points.
...
\]
At every point where the contradictory antecedent is recovered, the same explosion
schema is valid for an arbitrary consequent, whether or not that consequent is
classical. The artifact packages both facts into one separation theorem.

\begin{theorem}[Recovered explosion is not global LP explosion]
\label{thm:gate9-explosion-boundary}
There are formulas for which antecedent-classical-restricted LP explosion holds
without any recovery assumption on the consequent, while unrestricted LP consequence
fails:
\[
\boxed{
...
glut witness shows that antecedent-classical-restricted LP validates strictly more
formula pairs than unrestricted LP consequence. No global classical completeness
theorem, proof-system equivalence, arbitrary-CPEL-model theorem, or claim that every
formula is recovery-admissible is made.
