...
on \texttt{research/matroid-minors-gate4}, stacked on the preceding matroid gates;
the root module \texttt{PEL4.lean} imports the relevant development.
Appendix~\ref{app:formal} provides the selective claim-to-declaration map for the
established manuscript core. The matroid program additionally carries
separate declaration-level audits for the channel closure, topological bridge,
circuits, and explicit-ground minor layers. Audit selections are not a
...
\texttt{PEL4/PaperAxiomAudit.lean} and the 24-declaration MPFG inventory are
checked by CI against a strict allow-list. Only \texttt{propext},
...
The working revision's CI output is authoritative for exact dependencies.
...
\begin{verbatim}
git fetch origin
git switch research/matroid-minors-gate4
git pull --ff-only
lake build
lake env lean PEL4/MatroidEvidenceMinorAxiomAudit.lean
