...
\[
\begin{gathered}
  \text{cell membership},\quad \text{wall count},
  \\[-1pt]
  \text{crossing time},\quad \text{intermediate phase sequence}
\end{gathered}
\]
...
This distinction is now fully model-realized for weight-generated strong model
paths and probability-free modal formulas. Sequential diagonal changes pass
through genuine complete midpoint models in the forced adjacent phase.
Simultaneous change instead realizes one complete boundary model at
\((c,c)\); because threshold membership is inclusive, its categorical value
is \(\Bval\). Thus ``no open intermediate phase'' does not mean ``no model at
the crossing.''
