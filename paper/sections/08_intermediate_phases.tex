...
The theorem in this section classifies the explicit affine interpolation of the two support masses. Section~\ref{sec:model-paths} subsequently verifies a genuine path of complete strong 4-PEL models for weight-generated endpoints, fixed support-event membership, affine signed support masses, and their packaged affine complex coordinate for every probability-free modal formula. It then completes the composition: the crossing times, midpoint phases, and simultaneous boundary value are realized by actual complete models on that path. Formulas containing probabilistic belief remain a separate boundary because their event memberships may themselves change with the path parameter.
