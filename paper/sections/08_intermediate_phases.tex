...
The theorem in this section classifies the explicit affine interpolation of the two support masses. Section~\ref{sec:model-paths} subsequently verifies a genuine path of complete strong 4-PEL models for weight-generated endpoints and proves that every fixed event mass follows exactly such an affine trajectory. It also verifies fixed support-event membership and affine signed support masses for every probability-free modal formula. The remaining composition step is to package those two masses into the complex coordinate and state the crossing-order and intermediate-phase result directly about the complete strong model path. Formulas containing probabilistic belief remain a separate boundary because their event memberships may themselves change with the path parameter.
