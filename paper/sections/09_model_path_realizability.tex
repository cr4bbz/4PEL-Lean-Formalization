...
There is also a formula-level boundary. The affine theorem applies directly to a \emph{fixed event} \(S\). For an arbitrary modal formula containing probabilistic belief, the set of worlds that positively or negatively support the formula can itself vary with \(t\). In that case the event whose mass is measured is no longer fixed, so affine event mass alone does not imply affine \texttt{modalPositiveBeliefMass} or \texttt{modalNegativeBeliefMass}.
