...
\begin{proposition}[Weight-generated probability]
...
\subsection{Complete crossing classification on the model path}

Suppose the endpoint values of \(\Bel_i\varphi\) differ in both threshold
bits. Lean derives threshold straddling for both components of
\(z_\varphi\), constructs their unique unit-interval crossing times
\(t_+\) and \(t_-\), and proves that the corresponding coordinate hits occur
in the actual models \(M_{t_+}\) and \(M_{t_-}\).

\begin{theorem}[Complete strong-model crossing classification]
For every probability-free modal formula whose endpoint belief values differ
across two threshold walls, exactly one of the following model-realized cases
holds:
\begin{enumerate}[label=(\roman*)]
  \item \(t_+<t_-\), and the complete model at
  \(t_m=(t_++t_-)/2\) has the positive-first intermediate value;
  \item \(t_+=t_-\), and the complete model at the common time satisfies
  \(z_\varphi=(c,c)\) and \(\Bel_i\varphi=\Bval\);
  \item \(t_-<t_+\), and the complete midpoint model has the negative-first
  intermediate value.
\end{enumerate}
\statusverified
\end{theorem}

The value \(\Bval\) in the simultaneous case is not an extra continuity
assumption. It follows from the inclusive threshold convention: at the common
wall intersection both inequalities \(c\leq P^+\) and \(c\leq P^-\) hold.
Unlike the sequential cases, simultaneity has no open interval carrying an
adjacent intermediate phase; it has a single distinguished boundary model.
...
vary with \(t\). The separate update-level boundary asks whether the convex
intermediate models can themselves be generated by admissible
