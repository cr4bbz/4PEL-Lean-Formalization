...
A further verified layer now connects this geometry to complete probabilistic models. Nonnegative normalized rational point weights generate local measures satisfying an explicit finite-probability integrity contract. Convex interpolation preserves those distributions and makes the mass of every fixed event affine in the interpolation parameter. The local paths can then be packaged into complete strong 4-PEL models while worlds, accessibility, atomic valuation, and threshold remain fixed.

For every probability-free modal formula, the two signed support masses form an affine complex coordinate on this complete model path. If the endpoint belief values differ across both walls, Lean constructs their unique crossing times directly on the path. Positive-first and negative-first order realize the corresponding forced adjacent phase in a complete midpoint model. If the crossings are simultaneous, the common model has support coordinate \((\thr,\thr)\) and belief value \(\Bval\) under the inclusive Lockean convention.
...
\longrightarrow \text{strong model path}
\\[-1pt]
\longrightarrow \text{affine formula-support coordinate}
\longrightarrow \text{threshold sides}
\\[-1pt]
\longrightarrow \text{wall crossings}
\longrightarrow \text{crossing order}
\\[-1pt]
...
Model-valued convex paths and their complete crossing classification are now verified for weight-generated strong endpoint models on a common semantic skeleton and the full probability-free modal fragment. This does not yet imply that every legacy conditionalization witness has such a strong representation, that every intermediate point is itself generated by conditionalization of the source model, or that support events defined by formulas containing probabilistic belief remain fixed. The rational affine crossing theorem also remains distinct from an abstract intermediate-value theorem over arbitrary continuous paths. These belief-containing, update-level, and topological questions are the next mathematical boundaries.
