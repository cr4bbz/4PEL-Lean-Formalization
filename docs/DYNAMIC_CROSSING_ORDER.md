...
The downstream **intermediate-vertex classification** is now verified in
`PEL4/ModalDynamicsIntermediatePhase.lean`: when `tp != tn`, the rational
interval between the two unique crossing times occupies the adjacent FDE phase
obtained by changing exactly the first-crossed threshold coordinate. For
example, along the support interpolation:
...
This theorem remains an affine rational support-path result.  The later
`PEL4/ComplexModelCrossing.lean` lift realizes its crossing times and sequential
midpoints as complete `StrongProbabilityModel` states for probability-free
formulas.  Neither result claims that the path is generated by admissible
conditionalization or quantifies over arbitrary continuous paths.
