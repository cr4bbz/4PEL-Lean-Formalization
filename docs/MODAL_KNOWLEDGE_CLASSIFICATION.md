...
## 8. Verified dynamic phase layer

`PEL4/ModalDynamicsPhaseClassification.lean` is verified through the repository
root build.
...
The module also packages the fracture and restoration models as witnesses that
admissible conditionalization realizes both off-diagonal transitions.  Later
modules extend this layer with complete reachability, threshold crossing,
crossing order, intermediate phases, and strong-model realizability for the
...
1. characterize a maximal path-invariant fragment containing selected `bel`
   formulas;
2. determine when convex strong-model paths are generated by admissible
   conditionalization;
3. prove sharper necessity and minimality results for the modal frame
   correspondences;
4. compare the resulting dynamic correspondence pattern with four-valued
   dynamic epistemic logics before making novelty claims.
