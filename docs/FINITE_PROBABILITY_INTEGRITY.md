...
## Completed successor chain

The repository has completed the planned sequence:
...
Accordingly, the probability-free crossing fragment now has a verified
model-level epistemic trajectory rather than only a support-mass interpolation.
The remaining boundary is different: the convex path is not yet shown to be
generated by admissible conditionalization, and formulas containing `bel` may
change their own support events along it.
